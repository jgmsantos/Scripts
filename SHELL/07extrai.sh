...
lista_arquivos="prec.anual.nc nf.anual.nc"  # Lista de arquivos a serem processados. Aqui coloque o nome dos arquivos com a sua extensão.
...
do
      ...
      while read linha ; do
            ...
            nome_variavel=$(echo ${nome_arquivo} | cut -d / -f3 | sed 's/.nc//g')  # Pega o nome da variável. Exemplo, o nome é prec.anual.nc, será armazenado apenas o nome "prec.anual".

            echo "${nome_variavel} - Lat: ${lat} - Lon: ${lon}"

            # O arquivo de saída terá 3 colunas: latitude longitude valor.
            cdo -s -outputtab,nohead,date,lat,lon,value -remapnn,lon="$lon"_lat="$lat" ${DIR_INPUT}/${nome_arquivo} > ${DIR_OUTPUT}/${nome_variavel}.txt
